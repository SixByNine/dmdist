...
c	farm:	factors that multiply n_e^2 when calculating SM
	...
	real negc0, Fgc0
        common /gcparms/ negc0, Fgc0
           ...
           CALL get_environment_variable("NE2001PATH", nepath)
	   open(11,file=trim(nepath)//'gal01.inp',status='old')
	   read(11,*)
...
c read arm parameters:
        CALL get_environment_variable("NE2001PATH",nepath)
        open(11, file=trim(nepath)//'ne_arms_log_mod.inp', status='old')
...
c     parameter (hgc=0.026)
     ...
     .                na,ha,wa,Aa,Fa 
      ...
      if(first) then
        CALL get_environment_variable("NE2001PATH", nepath)
        open(11, file=trim(nepath)//'ne_gc.inp', status='old')
          read(11,*)
          read(11,*) xgc, ygc, zgc
	  read(11,*) rgc
	  read(11,*) hgc
